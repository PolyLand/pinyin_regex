...
# 锚点匹配
pinyin_regex_match("^yin", "音乐很好听")  # True (行首)
pinyin_regex_match("ting$", "音乐很好听")  # True (行尾)
pinyin_regex_match("^yin.*ting$", "音乐很好听")  # True (完整匹配)
```

### 调试工具

```python
from pinyin_regex.debug import dump_nfa, debug_pattern, PerformanceProfiler
...
print(f"匹配步骤: {debug_info['step_count']}")

# 性能分析
profiler = PerformanceProfiler()
duration = profiler.profile_matching("yinyue", "音乐")
print(f"匹配耗时: {duration:.6f}秒")
```
